...
import { ARTISTS_TREE } from '../../../common/endpoints'
...
class App extends Component {
        ...
        return {
            headerTitle: navigation.getParam('title',
                <View style={{ alignItems: 'center', justifyContent: 'center', width: width * .6 }}>
                    <Text style={styles.mediaText}>{i18n.t("Artists")}</Text>
                </View>
            ),
            headerTitleStyle: {
                flex: 1,
    ...
    getData() {
        ...
        Api('get', ARTISTS_TREE + `?countryId=${this.state.country.countryId}` + `&language=${language}`)
            .then((responseJson) => {
                if (responseJson) {
    ...
    searchItem(keyword) {
        ...
        Api('post', ARTISTS_TREE + `?language=${language}` + `&countryId=${this.state.country.countryId}`, data)
            .then((response) => {
                console.log("response", response);
                if (response) {
                    this.setState({
                        artist: response, isSearchLoading: false, searchKeyword: keyword
                    })
                }
                else {
                    this.setState({
    ...
    renderArtists({ item }) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate("ArtistsTreeDetail", { item: item })} style={[styles.mediaContainer, this.props.lang == 'ar' ? { flexDirection: 'row' } : { flexDirection: 'row-reverse' }]}>
                        ...
                        <View style={styles.searchBox}>
                            <View style={styles.search}>
                                ...
                                {/* <View style={styles.countryIcon}>
                                    <Image source={{uri: this.state.country.countryPicture}} style={styles.countryImage} resizeMode="cover"/>
                                </View> */}
                                <View style={styles.country}>
                                    <Text numberOfLines={2} style={styles.countryText}>{this.state.country.title} ({this.state.artist.length})</Text>
                                </View>
                            </View>
                        </View>
