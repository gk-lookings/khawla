...
class App extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            page: 1,
            isLastPage: true,
            courseData: this.props.navigation.getParam('courses', null),
    ...
    getData() {
        ...
        Api('get', LESSON_COURSES + `?language=${language}&courseId=${id}`)
            .then((response) => {
                if (response) {
    ...
    onPressItem({ item, index }) {
        if (this.props.user) {
            this.props.navigation.navigate("OnlineDetail", { onlineData: item, onlineIndex: index })
        }
        else if (item.isLoginRequired) {
            this.setState({ isVisible: true })
        }
        else {
            this.props.navigation.navigate("OnlineDetail", { onlineData: item })
        }
    }
