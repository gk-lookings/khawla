...
class App extends Component {
    ...
    componentDidMount() {
        ...
        Api('get', ARTISTS_TREE + `?artistId=${this.state.artist.artistId}` + `&language=${language}`)
            .then((responseJson) => {
                if (responseJson) {
                    console.log("Artists..gggggetai.", responseJson)
                    this.setState({
    ...
    onPressItem(item) {
        var language = this.props.lang == 'ar' ? 1 : 2
        Api('get', ARTISTS_TREE + `?artistId=${item.artistId}` + `&language=${language}`)
            .then((responseJson) => {
                if (responseJson) {
    ...
    renderMasters({ item }) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.push("ArtistsTreeDetail", { item: item })} style={styles.mastersItem}>
