...
import { StyleSheet, View, TouchableOpacity, Text, SafeAreaView, BackHandler, Alert } from 'react-native'
...
class App extends Component {
    ...
    onVerify() {
        ...
        if ( page === null || page === "password"){
            Api('post', VERIFY_OTP, formdata)
            .then((response) => {
                if (response && response.status && response.statusCode == 200) {
                    this.setState({ isLoading: false, errormessage: null,  })
                    // console.log('page...........', page)
                    if (page === "password") {
                        this.props.navigation.navigate('ResetPassword', { email: this.state.email, otp: this.state.code })
                    }
                    else {
                        this.props.dispatch({ type: 'SIGNUP_FETCHING_SUCCESS', user: this.state.user })
        ...
        if ( page === "changemail"){
            ...
            Api('post', CHANGE_EMAIL, formdata)
            .then((response) => {
                if (response && response.status && response.statusCode == 200) {
                  this.setState({ isLoading: false, errormessage: null })
            ...
            <SafeAreaView style={styles.container}>
                <KeyboardAwareScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
                    <View style={styles.header}>
                        ...
                        <Text style={styles.subtitle}>An one time password has been sent to <Text style={styles.email}>{this.state.email}</Text>.Please  enter the verification code.</Text>
                    </View>
                    <View style={{ flex: .3 }}>
                        <CodeInput
                            ref='codeInputRef'
                    ...
                    <TouchableOpacity onPress={this.onVerify} style={styles.button} disabled={this.state.code < 4 ? true : false} >
                        <Text style={styles.signin}>{this.state.isLoading ? 'Please wait...' : 'Verify'} </Text>
