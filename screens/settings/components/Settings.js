...
class App extends Component {
  ...
  showMenu(intent, assestId) {
    if (intent == "addPhoto") {
      this.setState({ selectedAssest: assestId }, () =>
    ...
    <View style={styles.content}>
      ...
      {options.map((item) => (
        <View key={item.key} style={styles.buttonContainer}>
          ...
          <TouchableOpacity
            style={styles.circle}
            onPress={() => this.setState({ language: item.key })}
          >
            {this.state.language === item.key && (
  ...
  renderModalChangeMail = () => (
    <View style={styles.content}>
      <View style={styles.modalClose}>
        <TouchableOpacity
          onPress={() => this.setState({ isVisibleChangeMail: false })}
        >
          <Icon name="ios-close-circle-outline" size={30} color="black" />
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.txtinput}>
        <TextInput
          ...
          onChangeText={(text) =>
            this.validate(text, "email", this.setState({ error: null }))
          }
  ...
  changeLanguage() {
    ...
    if (!this.state.user) {
      ...
      Api("post", CHANGE_LANGUAGE, formdata).then((response) => {
  ...
  changeUsername() {
    ...
    } else this.setState({ userEmpty: true });
  }
  changeEmail() {
    if (this.state.email.length > 0) {
      ...
      formdata.append("appId", 1);
      Api("post", SEND_OTP_EMAIL_CHANGE, formdata).then((response) => {
    ...
    } else this.setState({ fieldError: true });
  }
  ...
  changePassword() {
    ...
    if (
      ...
      Api("post", CHANGE_PASSWORD, formdata).then((response) => {
        ...
        } else Alert.alert(response.errormessage);
      });
    } else this.setState({ fieldError: true });
  }
  ...
  logout() {
    if (this.props.user.auth == 4) {
      ...
      Api("post", LOGOUT, { action: "logout" }).then((response) => {
        console.log("response logout", response);
        if (response.status) this.props.dispatch(resetUser());
      });
    } else {
      Api("post", LOGOUT, { action: "logout" }).then((response) => {
        console.log("response logout", response);
  ...
  delete() {
    ...
    Api("post", DELETE, formdata).then((response) => {
      ...
      <SafeAreaView style={styles.mainContainer}>
        <ScrollView keyboardShouldPersistTaps="handled">
          <View style={styles.header}>
            ...
            <View style={styles.profile}>
              {this.state.user && this.state.user.profile_pic ? (
                <TouchableOpacity
                  disabled={this.props.user && this.props.user.auth == 4}
                  onPress={() => this.showMenu("addPhoto")}
                >
                  <Image
                    source={
                      this.state.image != null
                        ...
                        : {
                            uri: `${this.state.user.profile_pic}&random=${
                              this.state.cache
                            }`,
                          }
                    }
          ...
          <View style={styles.contentContainer}>
            {!this.props.user && (
              <TouchableOpacity
                ...
                onPress={() =>
                  this.props.navigation.navigate("Login", { page: "Settings" })
                }
              >
                <Text style={styles.loginto}>
                  Login to Khawla Foundation{"\n"}
                  <Text style={styles.loginsub}>for better experience</Text>
                </Text>
                <IconMaterial
            ...
            {this.props.user && this.props.user.auth != 4 && (
              <TouchableOpacity
                style={styles.notification}
                onPress={() => this.setState({ isVisibleUser: true })}
              >
                <Text style={styles.notificationText}>
                  {i18n.t("Change_Username")}
                </Text>
                <IconMaterial
                  name="keyboard-arrow-right"
                  size={28}
            ...
            {this.props.user && this.props.user.auth != 4 && (
              <TouchableOpacity
                style={styles.notification}
                onPress={() => this.setState({ isVisibleChangeMail: true })}
              >
            ...
            {this.props.user && this.state.activeAccount ? (
              <TouchableOpacity
                style={styles.notification}
                onPress={() => this.setState({ isVisibleUserActive: true })}
              >
              ...
              this.props.user && (
                <TouchableOpacity
                  style={styles.notification}
                  onPress={() => this.setState({ isVisibleUserDelete: true })}
                >
            ...
            {this.props.user && (
              <TouchableOpacity
                ...
                onPress={() => this.setState({ isVisibleLogout: true })}
              >
          ...
          <Modal
            ...
            backdropOpacity={0.02}
            onBackButtonPress={() =>
              this.setState({ isVisibleChangeMail: false })
            }
            onBackdropPress={() =>
              this.setState({ isVisibleChangeMail: false })
            }
          ...
          <Modal
            ...
            onBackButtonPress={() =>
              this.setState({ isVisibleUserDelete: false })
            }
            onBackdropPress={() =>
              this.setState({ isVisibleUserDelete: false })
            }
          ...
          >
            <View style={styles.modalContainer}>
              <View style={styles.modalHeader}>
                ...
                />
                <Text style={{ fontFamily: FONT_MULI_REGULAR }}>
                  Your account data will be deleted in 30days, you can suspend
                  deletion with in this time period
                </Text>
              </View>
              ...
              <View style={styles.modalFooter}>
                <TouchableOpacity
                  ...
                  onPress={() => this.setState({ isVisibleUserDelete: false })}
                >
                  <Text style={styles.cancel}>CANCEL</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() =>
                    this.setState({ isVisibleUserDelete: false }, this.delete)
                  }
                  style={styles.button}
                >
          ...
          <Modal
            ...
            animationOutTiming={300}
            onBackButtonPress={() =>
              this.setState({ isVisibleUserActive: false })
            }
            onBackdropPress={() =>
              this.setState({ isVisibleUserActive: false })
            }
          ...
          >
            <View style={styles.modalContainer}>
              <View style={styles.modalHeader}>
                ...
                />
                <Text style={{ fontFamily: FONT_MULI_REGULAR }}>
                  Your delete request will be cancelled and Account will be
                  Restored
                </Text>
              </View>
              ...
              <View style={styles.modalFooter}>
                ...
                <TouchableOpacity
                  onPress={() =>
                    this.setState({ isVisibleUserDelete: false }, this.active)
                  }
                  style={styles.button}
                >
          ...
          <Modal
            ...
            animationOutTiming={300}
            onBackButtonPress={() =>
              this.setState({ activeAccountModal: false })
            }
          ...
          >
            <View style={styles.modalContainer}>
              ...
              <TouchableOpacity
                ...
                onPress={() => this.setState({ activeAccountModal: false })}
              >
          ...
          >
            <View style={styles.modalContainer}>
              ...
              <View style={styles.modalFooter}>
                <TouchableOpacity
                  style={styles.buttonCancel}
                  onPress={() => this.setState({ isVisibleLogout: false })}
                >
                  <Text style={styles.cancel}>CANCEL</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  onPress={() =>
                    this.setState({ isVisibleLogout: false }, this.logout)
                  }
                  style={styles.button}
                >
          ...
          >
            <SafeAreaView style={{ flex: 1 }}>
              ...
              >
                ...
                <Text style={{ marginTop: 10, fontFamily: FONT_PRIMARY }}>
                  Loading...
                </Text>
              </View>
            </SafeAreaView>
