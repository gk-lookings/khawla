...
import {  ARTISTS_WORLD_CAT } from '../../../common/endpoints'
import i18n from '../../../i18n'
import Icon from 'react-native-vector-icons/MaterialCommunityIcons'
import ArtistsWorld from '..'
...
class App extends Component {
    ...
    getData() {
        ...
        Api('get', ARTISTS_WORLD_CAT + `?language=${language}`)
            .then((responseJson) => {
                if (responseJson) {
    ...
    renderArtists({ item }) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate("ArtistList", { item: item })} style={[styles.mediaContainer, this.props.lang == 'ar' ? { flexDirection: 'row' } : { flexDirection: 'row-reverse' }]}>
    ...
    renderArtistGrid({ item }) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate('ArtistList', { item: item })} style={styles.itemContainerGrid}>
