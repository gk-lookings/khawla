...
import { EVENT_VIDEOS } from '../../../common/endpoints'
...
class App extends Component {
    ...
    getData() {
        ...
        Api('get', EVENT_VIDEOS + `?eventId=${this.state.events.eventId}&language=${language}` + `&page=${page}`)
            .then((response) => {
    ...
    renderLectureGrid({ item }) {
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate("VideoDetail",{event: this.state.events, video: item})} style={styles.itemContainerGrid}>
                <Image borderRadius={10} source={{ uri: this.state.events.eventCover }} style={styles.imageStyleGrid} />
