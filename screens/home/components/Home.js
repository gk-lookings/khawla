...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      nav: this.props.navigation.state.routeName,
    };
      ...
      {options.map((item) => (
        <View key={item.key} style={styles.buttonContainer}>
          ...
          <TouchableOpacity
            style={styles.circle}
            onPress={() =>
              this.setState({ language: item.key, isVisible: false }, () =>
                this.changeLanguage()
              )
            }
          >
            {this.props.lang === item.key && (
  ...
  renderBanner({ item }) {
    return (
      <View>
        <Image
          source={{ uri: item.bannerPictureApp }}
          resizeMode="repeat"
          style={styles.banner}
        />
  ...
  renderIcons({ item, index }) {
    return (
      <Animatable.View delay={index * 50} animation="fadeInUp">
        <TouchableOpacity
          onPress={() => this.onPressIcon({ item: item })}
          style={styles.icons}
        >
          <View style={styles.iconImage}>
            <View style={styles.shadow}>
              <Image
                source={{ uri: item.modulePicture }}
                style={styles.iconImags}
              />
            </View>
          </View>
          <View style={styles.iconTextbox}>
            <Text numberOfLines={3} style={styles.iconText}>
              {item.moduleName}
            </Text>
          </View>
        </TouchableOpacity>
      ...
      >
        {!this.state.isLoading ? (
          <View>
            ...
            {this.props.banner && (
              <TouchableWithoutFeedback
                onPress={() => this.props.navigation.navigate("UpcomingEvents")}
              >
                <ImageBackground
                  resizeMode="stretch"
                  source={{ uri: this.state.sequence[0].bannerPictureApp }}
                  style={styles.banner}
                >
                  <TouchableOpacity
                    style={styles.close}
                    onPress={() => this.props.dispatch(setBanner())}
                  >
                    <AntDesign
                      name="closecircleo"
                      size={20}
