...
class App extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            playing: false,
            artistDetail: '',
            artistData: this.props.navigation.getParam('audio', null),
            item: '',
            audioId: this.props.navigation.getParam('audioId', null),
        }
        this.vimeoPlay = this.vimeoPlay.bind(this);
    }
    componentDidMount() {
        this.props.navigation.setParams({ onPress: this.onPress })
        var id = this.state.audioId == null ? this.state.artistData.audioId : this.state.audioId
        var language = this.props.lang == 'ar' ? 1 : 2
        Api('get', AUDIO_GALLERY + `?language=${language}&audioId=${id}`)
            .then((response) => {
    ...
    vimeoPlay() {
        if (this.state.artistDetail && this.state.artistDetail.audioURL.includes('vimeo')) {
            if (this.state.artistDetail.audioURL) {
                var url = this.state.artistDetail.audioURL;
                        ...
                        {this.state.playing && this.state.videoLink != '' ?
                            ...
                            <TouchableOpacity style={styles.videoContainerMain} onPress={() => this.setState({ playing: true })}>
                                <View style={styles.imageVideoBG}>
                                    <Image style={styles.imageVideoImage} source={{ uri: artist.audioPicture }} />
                                </View>
                                <FontAwesome5 style={styles.videoIcon} name='play' size={40} color={PRIMARY_COLOR} />
                            </TouchableOpacity>}
                        <View style={styles.videoTitleContainer}>
                            <Text numberOfLines={1} style={styles.videoTitle}>{artist.audioTitle}</Text>
                        </View>
                    </View>
                </ScrollView>
            </SafeAreaView>
