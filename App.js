...
class App extends Component {
  async componentDidMount() {
    ...
    if (notificationOpen) {
      console.log('__--------____---_-------open___+-=-=-=-_=-=-=-=-', notificationOpen.notification)
      ...
      if (notification.data.itemType == 2) {
        NavigationService.push('EventDetail', { eventId: notification.data.itemId })
      }
      if (notification.data.itemType == 3) {
        NavigationService.push('ProgrammeDetail', { programmeId: notification.data.itemId })
      }
      if (notification.data.itemType == 4) {
        NavigationService.push('LatestFeedsDetail', { latestFeedId: notification.data.itemId })
      }
      if (notification.data.itemType == 5) {
        NavigationService.push('OnlineSection', { courseId: notification.data.itemId })
      }
      if (notification.data.itemType == 6) {
        NavigationService.push('CollectionDetail', { collectionId: notification.data.itemId })
      }
      if (notification.data.itemType == 7) {
        NavigationService.push('ArtistDetail', { artistId: notification.data.itemId })
      }
      if (notification.data.itemType == 8) {
        NavigationService.push('Library')
      }
      if (notification.data.itemType == 9) {
        NavigationService.push('AudioDetail', { audioId: notification.data.itemId })
      }
      ...
      if (notification.data.itemType == 13) {
        NavigationService.push('KidsCornerLessons', { courseId: notification.data.itemId })
      }
      if (notification.data.itemType == 14) {
        NavigationService.push('CollabrationDetail', { collabrationId: notification.data.itemId })
      }
      if (notification.data.itemType == 15) {
        NavigationService.push('ArticleDetail', { articleId: notification.data.itemId })
      }
